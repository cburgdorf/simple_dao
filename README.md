...
>Not that `TO` is set to the address of the rocket pool contract and `WEI` is set to the value of the proposed tx. The `DATA` is set to the calldata that we aquired in step 3.
...
This means that the proposed transaction is indeed calling `deposit()` on the rocket pool deposit contract with a value of `0.1` ETH.
